...
import com.jettmarks.routes.client.bean.RouteRequestBase;
import com.jettmarks.routes.client.bean.RouteRequestRouteName;
import com.jettmarks.routes.client.bean.SuitabilityRating;
import com.jettmarks.routes.client.service.GetDisplayElementsAsync;
 ...
 * of records into a bunch of EncodedTracks on a Map.
 * 
...
public class ServiceWrapper
{
  ...
  {
    GeoXmlLoadCallback callback = new GeoXmlLoadCallback()
    {
    ...
    GeoXmlOverlay.load(
        "http://gpx.cycling.jettmarks.com/master/CyclingRides.kmz", callback);
  }
   */
...
//      {
//        String fullURL = ((Element) rdfLines.item(i)).getAttribute("rdf:resource");
...
//    saveRoute(route.getName(), route.getGpxContent(), tags);
   ...
   * 
   * Calls the service that also updates the database as well as generating the
   * GPX on the file system.
   * 
   * @param route - Route that supplies the GPX data and the name of the route.
   * @param tags - How to classify the route.
   * @param rating - SuitabilityRating that indicates this is a SegmentedRoute 
   *   that should be saved as a SegmentedRoute.
   */
  public static void saveRoute(Route route, String tags[], SuitabilityRating rating)
  {
...
//    String gpxContent = route.getGpxContent();
    ...
    routeSaveService.saveAs(newRouteName+".gpx", tags, gpxContent, rating, callback);
  }
                               ...
                               String[] tags)
  {
    ...
    AsyncCallback<Integer> callback = new AsyncCallback<Integer>()
    {
      public void onFailure(Throwable caught)
      {
        Window.alert(ServiceWrapper.class.getName()+": Route Save Failed: " 
            + caught.toString());
      }
    ...
    routeSaveService.saveAs(newRouteName+".gpx", tags, gpxContent, callback);
  }
...
//    saveRouteThenShow(route.getName(), route.getGpxContent());
   ...
   * 
   * Uses the appropriate inner class for constructing
   * an appropriate client bean from the given DisplayElement.
   * 
   * @param nextRoute
   */
...
//        Window.alert("Retrieving "+displayElement);
      ...
      else if ("SuitabilitySegment".equals(nextRoute.getClassName()))
      {
        ...
        AsyncCallback<SuitabilitySegmentDE> callback = 
          new SuitabilitySegmentAsyncCallback();
   ...
   * @author jett
   */
  public class SuitabilitySegmentAsyncCallback
                                              implements
                                                    ...
                                                    AsyncCallback<String[]>
  {
    ...
    {
      // Make sure we found something before trying to create RouteRequests
      if (routeList == null)
      {
        if (tagList[0] == null)
        {
  ...
  /**
     * Handles the callback from the server after we've asked it to give us a list
     * of the DisplayElements.
     *
     * @author jett
     */
    public final class DisplayElementsAsyncCallback 
    		implements AsyncCallback<List<DisplayElementDTO>>
    {
