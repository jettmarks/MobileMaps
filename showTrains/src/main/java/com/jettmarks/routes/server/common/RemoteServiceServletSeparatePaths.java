...
public class RemoteServiceServletSeparatePaths extends RemoteServiceServlet
{
			...
			String   strongName) 
  {
    ...
    if (modulePath == null ) {
      ...
      //Set up input stream for serialization policy file, based on / servlet call. (07/11/08 - Danny)localhost.2011-08-17.log
      if(contextPath.equals("/servlet")) {
        ...
        // strict prefix.
        getServletContext().log("modulePath: "+modulePath + " ContextPath: "
            + contextPath);
	      String contextRelativePath;
        if (modulePath.contains(contextPath))
        {
	        contextRelativePath = modulePath.substring(contextPath.length());
        }
        else
        {
        ...
        // Adding the context of '/showRoutes' in case it is missing ('/' only)
