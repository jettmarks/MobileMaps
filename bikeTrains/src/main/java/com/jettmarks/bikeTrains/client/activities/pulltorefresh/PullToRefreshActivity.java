...
public class PullToRefreshActivity extends DetailActivity {
	...
	public void start(AcceptsOneWidget panel, EventBus eventBus) {
		...
		final PullToRefreshDisplay display = clientFactory
		...
		PullArrowStandardHandler headerHandler = new PullArrowStandardHandler(
				display.getPullHeader(), display.getPullPanel());
				...
				new Timer() {
					...
					public void run() {
						...
						if (failedHeader) {
							...
							for (int i = 0; i < 5; i++) {
								list.add(0, new Topic("generated Topic "
										+ (counter + 1), counter));
								counter++;
							}
							display.render(list);
		...
		PullArrowStandardHandler footerHandler = new PullArrowStandardHandler(
				display.getPullFooter(), display.getPullPanel());
				...
				new Timer() {
					...
					public void run() {
						...
						if (failedFooter) {
							...
							for (int i = 0; i < 5; i++) {
								list.add(list.size(), new Topic(
										"generated Topic " + (counter + 1),
										counter));
								counter++;
							}
							display.render(list);
