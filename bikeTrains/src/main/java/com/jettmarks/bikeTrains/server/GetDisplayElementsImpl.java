...
import com.jettmarks.bikeTrains.client.service.GetDisplayElements;
import com.jettmarks.bikeTrains.server.bean.BikeTrainWrapper;
import com.jettmarks.bikeTrains.server.bean.DisplayElementWrapper;
import com.jettmarks.bikeTrains.server.bean.DisplayGroupWrapper;
import com.jettmarks.bikeTrains.server.bean.SuitabilitySegmentWrapper;
import com.jettmarks.bikeTrains.server.bt.RouteOnlyBikeTrain;
 ...
 * @author jett
 */
public class GetDisplayElementsImpl extends RemoteServiceServletSeparatePaths
		implements GetDisplayElements {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger
			.getLogger(GetDisplayElementsImpl.class);
   ...
   */
	...
	public List<DisplayElementDTO> getElementList(
			DisplayGroupDTO displayGroupDto) {
		...
		if ("test".equals(displayGroupDto.getDisplayName())) {
			...
			DisplayGroup dgExample = DisplayGroupWrapper
					.getDisplayGroup(displayGroupDto);
			...
			for (DisplayElement de : elements) {
				DisplayElementDTO deDto = DisplayElementWrapper
						.getDisplayElementDTO(de);
	...
	public BikeTrainDE getBikeTrain(DisplayElementDTO displayElement) {
		...
		EncodedTrack et = getRouteImpl.getEncodedTrack(
				bikeTrain.getRouteName(), "Local Drive", tags);
	 ...
	 * 
	 * @see
	 * com.jettmarks.routes.client.service.GetDisplayElements#getSuitabilitySegment
	 * (com.jettmarks.routes.client.bean.DisplayElement)
	 */
	public SuitabilitySegmentDE getSuitabilitySegment(
			DisplayElementDTO displayElement) {
		...
		SuitabilitySegment sSegment = ssDao.findById(displayElement
				.getSourceId());
		...
		// Unfortunate package switch forces handling the enum as different
		switch (sSegment.getRating()) {
		case GREEN:
			et.setRating(SuitabilityRating.GREEN);
			break;
		case YELLOW:
			et.setRating(SuitabilityRating.YELLOW);
			break;
		case RED:
			et.setRating(SuitabilityRating.RED);
			break;
		case UNRATED:
			et.setRating(SuitabilityRating.UNRATED);
			break;
		}

		// Assemble new Display Element for SuitabilitySegment
		SuitabilitySegmentDTO ssDto = SuitabilitySegmentWrapper
				.getSuitabilitySegment(sSegment);
	...
	public List<RouteOnlyDE> getRouteOnlyList() {
		...
		// TODO: This only handles three quarters; need to roll through added
		// ones too
		// List<DisplayElementDTO> displayElementList =
		// gdeImpl.getElementList(dgSummer);
